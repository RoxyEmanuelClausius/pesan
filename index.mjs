...
async function getStarted(){
  ...
  const rainbow = chalkAnimation.karaoke('selamat datang');
  setTimeout(async()=> {
     rainbow.stop()
     console.log(`Uverseni `)
...
async function startBot(){
  ...
  client.on("ready",async()=>{
    ...
    spinner.success({ text: `Logged in as ${client.user.tag} (${client.user.id})`})
     app.get('/',(r, s) => {
       s.send({message:"Uverseni",youtube_channel:"https://www.youtube.com/@uverseni"})
      }).post('/',async(r, s) => {
        s.send({
          message:"Uverseni",  youtube_channel:"https://www.youtube.com/@uverseni"
        })
     ...
     .listen(3000)
     console.log("\u001b[32m▣\u001b[0m \u001b[0mBot Run By \u001b[34;Roxy.Emanuel\u001b[0m")
  ...
  client.on("interactionTextInput",async(interaction)=>{
    if(interaction.customId.startsWith("opentextinput")){
      ...
      if(await questions_db.has({key:customId}) != true) return await interaction.reply({content:`:x: Kiriman ini tidak lagi tersedia`,ephemeral:true})
      ...
      interaction.fields.forEach((field)=>{
        embed.addField(data.questions[+field.custom_id.split("_")[0]].label,`\`\`\`${field.value}\`\`\``,false)
      })
