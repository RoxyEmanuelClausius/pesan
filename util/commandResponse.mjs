...
export default async function(client,interaction,config) {
  if(interaction.isButton()){
    if(interaction.customId.startsWith('apply_')){
       if(await questions_db.has({key:interaction.customId}) != true) return await interaction.reply({content:`:x: This submission is no longer available`,ephemeral:true})
       //if(await users_applys_db.has({key:`apply_${interaction.user.id}_${interaction.customId}`}) == true) return await interaction.reply({content:`:x:You have already submitted!`,ephemeral:true})
       ...
       data.questions.forEach((question,i) =>{
         components.push(new DiscordModal.TextInputField()
          ...
          .setPlaceholder("Answer the question here")
  ...
  if(interaction.isCommand()){
     ...
     if(interaction.commandName == "apply"){
       if(config.owernid != interaction.user.id) return  await interaction.reply({content:`:x:You do not have  Permission to do this`,ephemeral:true}) 
        if(args[0] == "delete_all"){
          await questions_db.overwrite("")
          await users_applys_db.overwrite("")
          await interaction.reply({
            content:`✅Executed Successfully cancel all submissions and delete all their data`,
            ephemeral:true
          }) 
        }
        if(args[0] == "set"){
